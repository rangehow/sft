    ...
    for i,o in zip(real_input,output):
        if test :
            if vllm:
                ...
                input_id=template.tokenizer.apply_chat_template(chat_dict,tokenize=False,add_generation_prompt=True)
                
            elif not vllm:
                chat_dict=reformate(i,None,template)
                input_id=template.tokenizer.apply_chat_template(chat_dict,tokenize=True,add_generation_prompt=True)
...
def gsm8k(instances,shot=False,mode=0,**kwargs):
    ...
    PROMPT="Q: {question}\nA:"
    if shot:
        real_input=[INSTURCTION+SHOT+PROMPT.format(question=q)      for q in instances["question"]]
    else:
        real_input=[INSTURCTION+PROMPT.format(question=q)      for q in instances["question"]]
    output=instances["answer"]
    
    if mode==0: # 因为现在没有考虑不带chat template的base模型训练（继续预训练，索性算了）
