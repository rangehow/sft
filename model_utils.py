...
def balanced_load(model_dir, num_devices, is_distillation=False, ratio=None, devices_idx=None):
    assert ratio is None or sum(ratio) == 1  , "ratio should be a list whose sum==1"
    ...
    def create_manual_device_map(model, num_devices, is_distillation=False, ratio=ratio, devices_idx=devices_idx):
        ...
        rotary_emb_params = sum(p.numel() for p in model.model.rotary_emb.parameters()) if hasattr(model.model, 'rotary_emb') else 0
        ...
        ratio_rotary_emb = math.ceil(rotary_emb_params / layer_params) if rotary_emb_params > 0 else 0
        ...
        if ratio is not None:
            ...
            layers_per_device = [total_layers // num_devices for _ in range(num_devices)]
...
if __name__=='__main__':
    ...
    balanced_load('/mnt/rangehow/models/Meta-Llama-3.1-8B-Instruct',4,is_distillation=False,ratio=ratio,devices_idx=[0,1,2,3])