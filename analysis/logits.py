        ...
        for d in dataset_list:
            ...
            if reuse_flag:
                ...
                with torch.inference_mode():
                    ...
                    collator=SpecialDataCollator(tokenizer=tokenizer,zero_prob=args.zero_prob,embedding_size = model.lm_head.weight.size()[0],div_mode=args.div_mode,mix=args.mix,mix_ratio=args.mix_ratio)
                    dataloader=DataLoader(dataset=train_dataset,batch_size=8,collate_fn=collator,num_workers=4,pin_memory=True )
                    ...
                    supervised_ratio_naive_label_similarity = supervised_similarity / naive_label_similarity
                    clm_ratio_naive_label_similarity =clm_similarity/naive_label_similarity
                    
                    config_str = f"dm_{args.div_mode}---zp_{args.zero_prob}---mix_{args.mix}---mr_{args.mix_ratio}"
                    result = {
